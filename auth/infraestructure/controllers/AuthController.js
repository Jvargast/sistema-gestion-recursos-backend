...
class AuthController {
    ...
    try {
      ...
      if (refreshToken) {
        try {
          ...
          await AuthService.removeRefreshToken(decoded.rut, refreshToken);
        } catch (error) {
          console.warn("No se pudo invalidar el refresh token durante logout:", error.message);
        }
      }
      if (req?.user?.id) {
        await AuditLogsService.logAction(
  ...
  async getAuthenticatedUser(req, res) {
    try {
      ...
      const permissionsData = await AuthService.listUserPermissions(user.id);
      if (!permissionsData) {
        return res.status(401).json({ error: "Usuario no autenticado" });
      }
      ...
      res.status(200).json({
        ...
        rol: permissionsData.rol || rol,
        permisos: permissionsData.permisos || permisos,
      });
    } catch (error) {
