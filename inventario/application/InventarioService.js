...
import ProductoRepository from '../infrastructure/repositories/ProductosRepository.js';
...
class InventarioService {
  ...
  async createInventario(data) {
    const producto = await ProductoRepository.findById(data.id_producto);
  ...
  async updateInventario(id_producto, data) {
    ...
    if (updated[0] === 0) throw new Error('No se pudo actualizar el inventario.');
