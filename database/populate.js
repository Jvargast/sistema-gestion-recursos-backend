...
async function populateDatabase() {
  try {
    ...
    await sequelize.transaction(async (transaction) => {
      ...
      const empresaCreada = await upsertBy(
        Empresa,
        { rut_empresa: empresaData.rut_empresa },
        empresaData,
        transaction,
      );

      const sucursalCreada = await upsertBy(
        Sucursal,
        {
      ...
      const empresaDemo = await upsertBy(
        Empresa,
      ...
      const sucursalDemo = await upsertBy(
        Sucursal,
        {
      ...
      await EstadoProducto.bulkCreate(estados, { ignoreDuplicates: true });
      ...
      for (const p of productosVenta) {
        const producto = await upsertBy(
          ...
          },
          {
            ...
            es_retornable: p.es_retornable || false,
          },
          transaction,
        );
        ...
        await upsertBy(
          Inventario,
          {
            ...
            id_sucursal: sucursalCreada.id_sucursal,
          },
          {
            cantidad: p.cantidad,
          },
      ...
      console.log("✅ Productos de venta sincronizados con inventario");
      ...
      console.log("✅ Insumos sincronizados con inventario");
      ...
      const estadosVentas = [
        ...
        },
        {
          ...
          descripcion:
            "La venta o pedido fue entregado exitosamente al cliente.",
          tipo_transaccion: "venta",
          es_inicial: false,
        },
        {
      ...
      await EstadoVenta.bulkCreate(estadosVentas, { ignoreDuplicates: true });
      ...
      const estadosPago = [
        {
          nombre: "Pendiente",
          descripcion: "El pago aún no se ha completado.",
        },
        {
          nombre: "Pagado",
      ...
      await EstadoPago.bulkCreate(estadosPago, { ignoreDuplicates: true });
      ...
      await MetodoPago.bulkCreate(metodosPago, { ignoreDuplicates: true });
